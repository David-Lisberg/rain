...
pub fn system_manage_animation_events(handle: &mut RainHandle) {
        ...
        if let Some(a) = animation {
            ...
            if let Some(animation_events) = &current_frame.events {
                if a.frame_progress == 0 {
                    let active_event = ActiveEvents {
                        entity: e,
                        events: animation_events.clone(),
                        frames_left: current_frame.duration,
                    };
    ...
    for (e, mut hitbox) in to_add_hitbox {
        if let Ok(mut q) = handle.world.query_one::<(Option<&Position2D>, Option<&Inventory>)>(e) {
            let (position, inventory) = q.get().unwrap();
