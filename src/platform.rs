...
impl RainHandle {
    ...
    pub fn draw_texture(&mut self, x: f32, y: f32, w: f32, h: f32, texture: Arc<Texture>, color: Color) {
        ...
        self.renderer.draw_pass.draw_calls.push(DrawCall::Mesh(
            Mesh {
                vertices: vec![
                    UIVertex { position: [ndc_x, ndc_h], uv: [0.0, 1.0], layer: texture.index, color: color_array },
                    UIVertex { position: [ndc_w, ndc_h], uv: [1.0, 1.0], layer: texture.index, color: color_array },
                    UIVertex { position: [ndc_x, ndc_y], uv: [0.0, 0.0], layer: texture.index, color: color_array },
                    UIVertex { position: [ndc_w, ndc_y], uv: [1.0, 0.0], layer: texture.index, color: color_array },
                ],
                indices: INDICES_RECTANGLE.to_vec(),
                material: texture,
            }
