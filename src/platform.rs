...
impl RainHandle {
    ...
    pub fn draw_rectangle(&mut self, x: f32, y: f32, w: f32, h: f32, color: Color) {
        let ndc1 = framebuffer_to_ndc((x, y), self.base_width, self.base_height);
        let ndc2 = framebuffer_to_ndc((x + w, y + h), self.base_width, self.base_height);
    ...
    pub fn draw_rectangle_ex(&mut self, x: f32, y: f32, w: f32, h: f32, color: Color, degrees: f32, origin: impl Into<Vec2>) {
        let origin: Vec2 = origin.into();
        let origin: (f32, f32) = (origin.x + x, origin.y + y);

        let p1 = rotate_around_pivot((x, y + h), origin, degrees);
        let p2 = rotate_around_pivot((x + w, y + h), origin, degrees);
        let p3 = rotate_around_pivot((x, y), origin, degrees);
        let p4 = rotate_around_pivot((x + w, y), origin, degrees);
    ...
    pub fn draw_texture(&mut self, x: f32, y: f32, w: f32, h: f32, texture: Arc<Texture>, tint: Color) {
        let ndc1 = framebuffer_to_ndc((x, y), self.base_width, self.base_height);
        let ndc2 = framebuffer_to_ndc((x + w, y + h), self.base_width, self.base_height);
